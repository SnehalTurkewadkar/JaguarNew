...
public class FindRoleUsingsortedWebTableElement {
		...
		public void VerifySortedElements()
		{
			...
			List<Object> empNameList = low.stream().map(s->s.getText()).collect(Collectors.toList());
			
			
			List<Object> listOfRoles = low.stream().filter(s->s.getText().contains("Anthony Nolan"))
										.map(s->getRole(s))
